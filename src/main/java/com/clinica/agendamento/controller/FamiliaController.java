...
import com.clinica.agendamento.dto.ResidenciaResponse;
...
public class FamiliaController {
        ...
        @GetMapping("/{id}")
        @PreAuthorize("hasAnyRole('ADMIN','UBS','ACS')")
        public ResponseEntity<FamiliaResponse> buscar(
        ...
        @PreAuthorize("hasAnyRole('ADMIN','UBS', 'ACS')")
        @PutMapping("/{id}")
        public ResponseEntity<Familia> atualizar(@PathVariable Long id, @RequestBody FamiliaRequest dto) {
        ...
        private FamiliaResponse toResponse(Familia familia) {
                ...
                ResidenciaResponse residencia = new ResidenciaResponse(
                                ...
                                familia.getResidencia().getEndereco());

                MoradorResponse responsavel = null;

                if (familia.getResponsavel() != null) {

                        responsavel = new MoradorResponse(

                                        familia.getResponsavel().getId(),
                                        familia.getResponsavel().getNome(),
                                        familia.getResponsavel().getCpf(),
                                        familia.getResponsavel().getDataNascimento(),
                                        familia.getResponsavel().getSexo(),
                                        familia.getResponsavel().getTelefone(),
                                        familia.getId(),
                                        familia.getResponsavel().getUltimaAtualizacao(),
                                        familia.getResponsavel().getAtualizadoPor()

                        );
                }
                return new FamiliaResponse(familia.getId(), residencia, responsavel, moradores);
        }
}